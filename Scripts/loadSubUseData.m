...
function [options,data] = loadSubUseData(options)
...
if ~isfield(options,'phcp_sfm_struct')
    ...
    options.phcp_sfm_struct = summarize_SFM_results( sfm_opt );
end
...
%% Configure the SFM data
% Grab the switch rates for  each included subject
data.sfm.dataAve = (squeeze(nanmean(...
    options.sfm_struct.illusory_task.Hz.all_data_plot(),3)));
data.sfm.dataAveLog = (squeeze(nanmean(...
    options.sfm_struct.illusory_task.Hz.all_data(),3)));
data.sfm.subjNum = [options.sfm_struct.subj_number{1};options.sfm_struct.subj_number{2};...
    options.sfm_struct.subj_number{3}];
% Also grab block data for SFM to do stats
data.sfm.dataAllLog = options.sfm_struct.illusory_task.Hz.all_data_plot;
...
for iI=3:5
    ...
    data.(data.runListUniqShort{iI}).tobacco.sfm.switchRate = data.sfm.dataAve(...
        ismember(data.sfm.subjNum,data.(data.runListUniqShort{iI}).tobacco.sfm.subjNum),sfmIdx);
    data.(data.runListUniqShort{iI}).tobacco.sfm.switchRateLog = data.sfm.dataAveLog(...
        ismember(data.sfm.subjNum,data.(data.runListUniqShort{iI}).tobacco.sfm.subjNum),sfmIdx);
    data.(data.runListUniqShort{iI}).tobacco.sfm.switchRateAllLog = data.sfm.dataAllLog(...
        ismember(data.sfm.subjNum,data.(data.runListUniqShort{iI}).tobacco.sfm.subjNum),sfmIdx,:);
    data.(data.runListUniqShort{iI}).tobacco.sfm.subjNum = data.sfm.subjNum(...
        ismember(data.sfm.subjNum,data.(data.runListUniqShort{iI}).tobacco.sfm.subjNum));
